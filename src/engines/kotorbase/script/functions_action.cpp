...
void Functions::actionCastSpellAtObject(Aurora::NWScript::FunctionContext &ctx) {
	...
	Action action(kActionCastSpell, target, spellID);
	caller->addAction(action);
}

void Functions::actionCastSpellAtLocation(Aurora::NWScript::FunctionContext &ctx) {
	int spellID = ctx.getParams()[0].getInt();
	Location *target = ObjectContainer::toLocation(ctx.getParams()[1].getEngineType());
	...
	float x, y, z;
	target->getPosition(x, y, z);
	Action action(kActionCastSpell, glm::vec3(x, y, z), spellID);
	caller->addAction(action);
}

void Functions::actionUseFeat(Aurora::NWScript::FunctionContext &ctx) {
	int featID = ctx.getParams()[0].getInt();
	Object *target = ObjectContainer::toObject(ctx.getParams()[1].getObject());
...
void Functions::actionWait(Aurora::NWScript::FunctionContext &ctx) {
	float seconds = ctx.getParams()[0].getFloat();
	Creature *caller = ObjectContainer::toCreature(ctx.getCaller());
	if (caller) {
		Action action(kActionWait);
		action.float1 = seconds;
		caller->addAction(action);
	}
}
...
void Functions::actionDoCommand(Aurora::NWScript::FunctionContext &ctx) {
	Creature *caller = ObjectContainer::toCreature(ctx.getCaller());
	if (caller) {
		Action action(kActionDoCommand);
		// Note: ActionExecutor needs to handle running the sub-script
		caller->addAction(action);
	}
}
...
void Functions::actionMoveToObject(Aurora::NWScript::FunctionContext &ctx) {
	...
	Creature *caller = ObjectContainer::toCreature(ctx.getCaller());
	if (caller && target) {
		Action action(kActionMoveToObject);
		action.object = target;
		action.range = range;
		action.bool1 = run;
		caller->addAction(action);
...
void Functions::actionGiveItem(Aurora::NWScript::FunctionContext &ctx) {
	Object *item = ObjectContainer::toObject(ctx.getParams()[0].getObject());
	Object *target = ObjectContainer::toObject(ctx.getParams()[1].getObject());
	Creature *caller = ObjectContainer::toCreature(ctx.getCaller());
	if (caller && item && target) {
		Action action(kActionGiveItem);
		action.object = item;
...
void Functions::actionTakeItem(Aurora::NWScript::FunctionContext &ctx) {
	...
	Creature *caller = ObjectContainer::toCreature(ctx.getCaller());
	if (caller && item && source) {
		Action action(kActionTakeItem);
		action.object = item;
...
void Functions::actionEquipMostDamagingMelee(Aurora::NWScript::FunctionContext &ctx) {
	Creature *caller = ObjectContainer::toCreature(ctx.getCaller());
	if (caller)
		caller->equipMostDamagingMelee();
}

void Functions::actionEquipMostDamagingRanged(Aurora::NWScript::FunctionContext &ctx) {
	Creature *caller = ObjectContainer::toCreature(ctx.getCaller());
	if (caller)
		caller->equipMostDamagingRanged();
}

void Functions::actionRest(Aurora::NWScript::FunctionContext &ctx) {
	Creature *caller = ObjectContainer::toCreature(ctx.getCaller());
	if (caller) {
		Action action(kActionRest);
		caller->addAction(action);
	}
}

void Functions::getIsResting(Aurora::NWScript::FunctionContext &ctx) {
	Creature *creature = ObjectContainer::toCreature(getParamObject(ctx, 0));
	ctx.getReturn() = (creature && creature->isResting()) ? 1 : 0;
}
...
void Functions::actionUseSkill(Aurora::NWScript::FunctionContext &ctx) {
	...
	Creature *caller = ObjectContainer::toCreature(ctx.getCaller());
	if (caller && target) {
		Action action(kActionUseSkill);
		action.object = target;
		action.type = static_cast<ActionType>(skill); // Re-use type as skill ID for now
		caller->addAction(action);
	}
}

void Functions::actionCircleObject(Aurora::NWScript::FunctionContext &ctx) {
	Object *target = ObjectContainer::toObject(ctx.getParams()[0].getObject());
	float radius = ctx.getParams()[1].getFloat();
	Creature *caller = ObjectContainer::toCreature(ctx.getCaller());
	if (caller && target) {
		Action action(kActionMoveToPoint); // Simple alias for now
		float x, y, z;
		target->getPosition(x, y, z);
		action.location = glm::vec3(x + radius, y, z);
		caller->addAction(action);
	}
}


void Functions::actionLockObject(Aurora::NWScript::FunctionContext &ctx) {
	Aurora::NWScript::Object *object = getParamObject(ctx, 0);
	if (object) {
		// Logic to lock the object would go here
		debugC(Common::kDebugEngineLogic, 1, "ActionLockObject: %s", object->getTag().c_str());
	}
}

void Functions::actionUnlockObject(Aurora::NWScript::FunctionContext &ctx) {
	Aurora::NWScript::Object *object = getParamObject(ctx, 0);
	if (object) {
		// Logic to unlock the object would go here
		debugC(Common::kDebugEngineLogic, 1, "ActionUnlockObject: %s", object->getTag().c_str());
	}
}

void Functions::actionCastFakeSpellAtObject(Aurora::NWScript::FunctionContext &ctx) {
	int spellId = ctx.getParams()[0].getInt();
...
void Functions::actionCastFakeSpellAtLocation(Aurora::NWScript::FunctionContext &ctx) {
	int spellId = ctx.getParams()[0].getInt();
	debugC(Common::kDebugEngineLogic, 1, "ActionCastFakeSpellAtLocation: spell %d", spellId);
}
