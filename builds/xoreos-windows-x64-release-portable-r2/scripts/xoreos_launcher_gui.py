...
GAME_DEFS = [
    {
        ...
        "target": "kotor",
        "markers": [("chitin.key", True), ("dialog.tlk", True), ("modules", True), ("override", False)],
    },
    {
        "label": "KotOR II",
...
class XoreosLauncherApp:
    ...
    def build_command(self, use_saved_target: bool, require_exe: bool) -> list[str]:
        ...
        command.append("--fullscreen" if self.mode_var.get() == "fullscreen" else "--windowed")
        if self.skip_videos_var.get():
            command.append("--skipvideos")
        return command
    ...
    def save_target(self) -> bool:
        try:
            ...
            if result["state"] == "error":
                raise ValueError("The selected folder does not look ready enough to save.")
    ...
    def launch_direct(self) -> None:
        try:
            result = self.validate_current_path(auto_only=True)
            if result["state"] == "error":
                raise ValueError("The selected folder does not look ready enough to launch yet.")
            self.prefer_saved_target = False
